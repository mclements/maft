c June 16 2002:
c splitting fetal effect to father and mother contributions
c
...
c
 	subroutine fetfm(param,npar,idata,m1,m2,loglik,prob)
	parameter (nn=15)
...
c if the first pregnancy exists
	  ...
	  call fetfmcov(param,npar,n11,n21,n1,n2,correl)
...
c
	subroutine fetfmcov(param,npar,n11,n21,n1,n2,correl)
	parameter (nn=15)
...
c 	      4=genetic-fetal-father, 5=fetal-mother 6=common sibling,
c             7=family environment
c             8=mother X late deliv 9= fetal X late delv
c             10=family env x late delivery
	sigm2 = param(3)
	sfetf2 = param(4)
	sfetm2 = param(5)
	sigs2 = param(6)
...
c total variance
	do i =1,nvar
	  do j=1,nvar
	  cov(i,j)=sigm2*Am(i,j) + sfetf2*Af(i,j) + sfetm2*Afmat(i,j)+
...
c
c from BROTHER-SISTER structure
c
 	subroutine bsfetfm(param,npar,idata,m1,m2,loglik,prob)
	parameter (nn=15)
	integer idata(m1,m2), INFIN(nn)
	DOUBLE PRECISION CORREL(nn*(nn-1)/2), LOWER(nn), UPPER(nn), 
     &    DELTA(nn), RELEPS, ABSEPS, ERROR, VALUE, MVINIT, MVFUNC
...
c if the first pregnancy exists
	  ...
	  call bsfetcov(param,npar,n11,n21,n1,n2,correl)
c          print*,(correl(i),i=1,nvar*(nvar-1)/2)
c
c start do loop
          do j=1,nvar
...
c end idat
	...
	subroutine bsfetcov(param,npar,n11,n21,n1,n2,correl)
	parameter (nn=15)
	real*8 param(npar),cov(nn,nn),correl(*)
c workspace
	real Ag(nn,nn),Am(nn,nn),Af(nn,nn),As(nn,nn),Abs(nn,nn)
	real Am2(nn,nn), Af2(nn,nn),Afam(nn,nn),Afmat(nn,nn)
...
c total variance
	do i =1,nvar
	  do j=1,nvar
	  cov(i,j)=sigm2*Am(i,j) + (sfetf2+ sfetm2)*Af(i,j)+
     &             sqrt(sfetf2)* sqrt(sfetm2)*Abs(i,j) +
     &             sigs2*As(i,j) + sigfam2*Afam(i,j) + 0.
c     &             sigm22*Am2(i,j) + sigf22*Af2(i,j)
	  enddo
	enddo
